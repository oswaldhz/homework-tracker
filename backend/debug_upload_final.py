...
def debug_upload_final():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.screenshot(path="upload_after_submit.png", full_page=True)
            print("[9] Screenshot saved: upload_after_submit.png")
            print()
            
            # Check if file appears in file manager
            print("[10] Checking file manager for uploaded file...")
            files_in_manager = file_manager.query_selector_all('.fp-file')
            print(f"Files in file manager: {len(files_in_manager)}")
            for f in files_in_manager:
            ...
            # Save HTML
            html = page.content()
            with open("upload_final_debug.html", "w", encoding="utf-8") as f:
                f.write(html)
            print("[11] HTML saved: upload_final_debug.html")
            
        except Exception as e:
            print(f"Error: {e}")
            import traceback
            traceback.print_exc()
            page.screenshot(path="upload_final_error.png")
        finally:
            browser.close()
